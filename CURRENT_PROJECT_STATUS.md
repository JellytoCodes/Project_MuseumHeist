...
> Rev14는 2026-08-18에 확정한 **Public Release 방향 재기준화**다. 2026-08-22 기준 C++ Rule/Authority/Replication 기반과 CCTV·Laser·Hold Button 공용 Blueprint Shell을 반영했고, M01/M02/M03 Release Map 인스턴스 연결·NavData Build·2인 Listen Server 두 판 자동화를 통과했다. 최종 VFX/Audio·사용자 화면/청음 PIE·Cook·3인/4인 Release Gate가 완료됐다는 의미는 아니다. 기존 W6/W7의 Solo·1P·Surface/Object 증거는 당시 구현의 역사적 검증으로 보존하며 Rev14 출시 Gate 통과로 재해석하지 않는다.
...
마지막 라이브 재조회: `2026-08-22 KST` (`TASK-W7-004~006` 완료, `W7 Gate=Pass` 쓰기·재조회 확인; `TASK-W8-001~008`은 기존 `미시작` 유지)
...
TASK-W7-004  완료    Stun Authority·HUD·후처리·Low-pass·상태 FX/Audio 슬롯·Cleanup / 최종 에셋은 별도 결합
TASK-W7-005  완료    Arrest / Rescue UI·오디오 호출·입력 복원·Lobby Cleanup / 최종 에셋은 별도 결합
TASK-W7-006  완료    Carry / Heavy Icon·속도·Footstep·Deposit·상태 FX/Audio 슬롯·Cleanup / 최종 에셋은 별도 결합
...
W7 Gate      Pass    에셋 비의존 Player Experience Integration 완료 / 최종 Niagara·Sound·Remote Pose는 별도 결합
...
### Rev14 Release Rebaseline — 3-Map Security Integration 2P Automation PASS
...
- 기존 자동화의 Object 성공 기대와 Public Solo Start 기대는 Rev14 경계에 맞게 재정렬했다. Security 사건 정책과 `SandBoxMap` 2인 Listen Server 자동화에 이어 M01/M02/M03 Public Contract 2인 두 판 자동화를 PASS했다.
...
- Widget은 변경하지 않았다. M01/M02/M03에는 각각 Painting Case 6개, Loose Loot 5개, CCTV/Laser/Hold Button 각 1개, Guard 1개와 Waypoint 4개를 배치했다. Laser는 FourStar Optional Painting, Hold Button은 해당 Laser를 직접 참조하며 Required Target은 Laser 뒤에 두지 않았다. 세 맵의 NavData를 재빌드·저장했다. M01의 Object Case 5개는 Deferred 호환 배치로 남지만 v1 Contract Assignment에서는 비활성이다. `/Game/Blueprints/World/Actors/Security` 공용 Blueprint Shell 3개는 EventGraph가 빈 상태이고 기본 Visibility와 Hold Button 눌림 Transform은 C++ `ApplyPresentation()`이 처리한다.
...
Widget                           NOT CHANGED
Release umap                     UPDATED / M01·M02·M03 security, exhibit, loot, guard/nav instances
SandBoxMap Test Instances         PASS / CCTV + Hold→Laser→FourStar Painting
Release Map Instance Links        PASS / 3 maps / CCTV 1 + Laser 1 + Hold 1 / direct references valid
Release Map NavData               PASS / M01·M02·M03 rebuilt and saved
Security VFX / Audio             PENDING / assignment hooks preserved
...
W7는 팀 가독성과 협동 피드백의 C++/복제/Input 기반과 Stun/Arrest/Carry HUD·Icon·Audio 호출을 마쳤다. 실제 HUD Widget Tree 동기화 후 2P TwoRuns와 전체 27/27 자동화를 통과했다. 2026-08-17 UE5 Manny와 `ABP_Unarmed`를 임시 Character/AnimBP 베이스로 연결했고, `/Game/Blueprints`의 Non-Blueprint Asset 29개를 `/Game/Assets`로 이동해 폴더 경계를 정리했다. 2026-08-18에는 상태별 Component를 늘리지 않고 재사용 Niagara/Audio Component 각 1개와 7개 non-Active 상태 Asset 슬롯을 추가했다. 2026-08-22 사용자 결정으로 최종 Niagara/Sound·Remote Pose 에셋을 W7 Gate에서 분리하고, 현재 작업 트리의 Editor Build·W7 11/11·M01 2P TwoRuns를 재검증한 뒤 `TASK-W7-004~006` 완료 및 `W7 Gate=Pass`를 Notion에 반영·재조회했다.
...
W7 Final Regression PASS / Saved/Automation/W7-AssetDeferred-FinalRegression / 11 of 11 / Failed 0
W7 Final 2P         PASS / Saved/Automation/W7-AssetDeferred-Final2P / 2 runs / Lobby clean reset / Failed 0
W7 Gate             PASS / Notion live write + re-fetch / 2026-08-22
```

범위 경계:

- NullRHI 자동화는 상태·복제·입력·Widget·Audio 호출·Cleanup 계약을 검증하지만 최종 에셋의 렌더링 품질과 주관적 청음을 증명하지 않는다.
- 최종 Niagara/Sound와 Remote Stun/Carry/Heavy Pose는 에셋 확보 후 별도 결합한다. 이 범위는 W7 완료 조건에서 제외했으며 W7 Gate를 다시 열지 않는다.
- 상태 VFX·전환 Sound 슬롯과 Lifecycle은 완료했고 미할당 슬롯은 정상 No-op이다. 에셋 할당 시 late-join, 반복 전환, Escaped one-shot과 실제 화면·청음만 별도 검증한다.
- 실제 2~3분 Escape 리듬은 W8-007로 통합했다.
- W7 Gate는 2026-08-22 `Pass`로 닫았다.
...
- M01/M02/M03 후속 배치 변경은 사용자 요청 범위와 현재 W8 Acceptance Criteria 안에서만 진행한다.
...
6. 현재 Revision의 `Project_MuseumHeistEditor Win64 Development` Full Build와 CCTV/Laser/Hold Button Blueprint 3종 Compile/Save/Data Validation은 PASS했다. 최신 빌드 증거는 `Saved/Logs/W8-ReleaseGameplay-FullBuild.log`, Blueprint 증거는 `Saved/Logs/W8-SecurityBlueprint-CompileSave.log`다.
7. 세 Security Blueprint의 EventGraph는 사용자 삭제 상태를 유지하며 다시 만들지 않는다. 최종 Mesh·Material·VFX·Audio를 할당할 때만 Class Defaults를 갱신하고 출처·라이선스·Notice를 함께 기록한다.
8. 테스트 전용 `SandBoxMap`의 CCTV·Laser·Hold Button·FourStar Painting 연결과 2인 Listen Server Security 자동화는 기능 PASS했다. 자동화 리포트는 SandBox의 RecastNavMesh 부재와 차단된 EOS 네트워크 경고 때문에 `Success With Warnings`이며 Gameplay assertion 오류는 0이다. 증거는 `Saved/Automation/W8-SandboxSecurity`, `Saved/Logs/W8-SandboxSecurity-FinalAudit.log`다.
...
Notion        2026-08-22 쓰기·재조회 PASS / TASK-W7-004~006 완료 / W7 Gate Pass
Test Log      TEST-W7-006 Pass / 에셋 비의존 자동 계약 + 2P TwoRuns / 최종 에셋은 별도 결합
```

### Remaining Evidence

- W7 상태 연출 에셋: 최종 Niagara/Sound·Remote Stun/Carry/Heavy Pose는 확보 후 별도 결합한다. W7 Task/Gate 완료 상태에는 영향 없음.
