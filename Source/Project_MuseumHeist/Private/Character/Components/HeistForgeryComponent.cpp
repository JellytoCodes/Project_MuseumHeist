...
namespace
{
	...
	void DecodeDxtColorBlock(
		const uint8* BlockData,
		const bool bForceFourColor,
		FColor OutColors[16])
	{
		const uint16 Color0 =
			static_cast<uint16>(BlockData[0] | (BlockData[1] << 8));
		const uint16 Color1 =
			static_cast<uint16>(BlockData[2] | (BlockData[3] << 8));
		FColor Palette[4];
		...
		{
			Palette[2] = FColor(
				static_cast<uint8>((2 * Palette[0].R + Palette[1].R) / 3),
				static_cast<uint8>((2 * Palette[0].G + Palette[1].G) / 3),
				static_cast<uint8>((2 * Palette[0].B + Palette[1].B) / 3),
				255);
			Palette[3] = FColor(
				static_cast<uint8>((Palette[0].R + 2 * Palette[1].R) / 3),
				static_cast<uint8>((Palette[0].G + 2 * Palette[1].G) / 3),
				static_cast<uint8>((Palette[0].B + 2 * Palette[1].B) / 3),
				255);
		}
		else
		{
			Palette[2] = FColor(
				static_cast<uint8>((Palette[0].R + Palette[1].R) / 2),
				static_cast<uint8>((Palette[0].G + Palette[1].G) / 2),
				static_cast<uint8>((Palette[0].B + Palette[1].B) / 2),
				255);
			Palette[3] = FColor(0, 0, 0, 0);
		}

		const uint32 Indices =
			static_cast<uint32>(BlockData[4])
			| (static_cast<uint32>(BlockData[5]) << 8)
			| (static_cast<uint32>(BlockData[6]) << 16)
			| (static_cast<uint32>(BlockData[7]) << 24);
		for (int32 PixelIndex = 0; PixelIndex < 16; ++PixelIndex)
		{
			OutColors[PixelIndex] =
				Palette[(Indices >> (PixelIndex * 2)) & 3];
		}
	}
	...
	void DecodeBcChannelBlock(const uint8* BlockData, uint8 OutValues[16])
	{
		...
		if (Palette[0] > Palette[1])
		{
			...
			{
				Palette[Index + 1] = static_cast<uint8>(
		...
		{
			Indices |= static_cast<uint64>(BlockData[2 + ByteIndex])
				<< (ByteIndex * 8);
		}
		for (int32 PixelIndex = 0; PixelIndex < 16; ++PixelIndex)
		{
			OutValues[PixelIndex] =
				Palette[(Indices >> (PixelIndex * 3)) & 7];
		}
	}

	bool DecodePlatformTextureIntensity(
		UTexture2D* Texture,
		TArray<uint8>& OutIntensity,
		int32& OutWidth,
		int32& OutHeight,
		EPixelFormat& OutPixelFormat)
	{
		...
		case PF_B8G8R8A8:
			...
			for (int32 PixelIndex = 0;
				PixelIndex < OutWidth * OutHeight;
				++PixelIndex)
			{
				OutIntensity[PixelIndex] = FMath::Max3(
					Bytes[PixelIndex * 4 + 2],
					Bytes[PixelIndex * 4 + 1],
					Bytes[PixelIndex * 4]);
			}
			break;
		case PF_R8G8B8A8:
		...
		case PF_BC5:
		{
			const int32 BlockBytes =
				OutPixelFormat == PF_DXT1 || OutPixelFormat == PF_BC4
					? 8
					: 16;
			const int32 BlocksX = FMath::DivideAndRoundUp(OutWidth, 4);
			const int32 BlocksY = FMath::DivideAndRoundUp(OutHeight, 4);
			if (BulkDataBytes
				< static_cast<int64>(BlocksX) * BlocksY * BlockBytes)
			{
				bDecoded = false;
				break;
			}
			...
			for (int32 BlockY = 0; BlockY < BlocksY; ++BlockY)
			{
				...
				{
					const uint8* Block = Bytes
						+ (BlockY * BlocksX + BlockX) * BlockBytes;
					FColor Colors[16];
					...
					for (int32 LocalY = 0; LocalY < 4; ++LocalY)
					{
						for (int32 LocalX = 0; LocalX < 4; ++LocalX)
						{
							...
							OutIntensity[Y * OutWidth + X] =
								OutPixelFormat == PF_DXT1
									|| OutPixelFormat == PF_DXT5
									? ColorLuminance(Colors[LocalIndex])
									: ChannelValues[LocalIndex];
						}
					}
				}
			}
	...
	bool DecodeReferenceMaskIntensity(
		UTexture2D* Texture,
		TArray<uint8>& OutIntensity,
		int32& OutWidth,
		int32& OutHeight,
		EPixelFormat& OutPixelFormat)
	{
		if (DecodePlatformTextureIntensity(
			Texture,
			OutIntensity,
			OutWidth,
			OutHeight,
		...
		FImage SourceImage;
		if (IsValid(Texture)
			&& FImageUtils::GetTexture2DSourceImage(Texture, SourceImage)
			&& SourceImage.SizeX > 0
			&& SourceImage.SizeY > 0)
		{
			SourceImage.ChangeFormat(
	...
	bool DecodePlatformTextureColors(
		UTexture2D* Texture,
		...
		EPixelFormat& OutPixelFormat)
	{
		...
		case PF_R8G8B8A8:
			...
			for (int32 PixelIndex = 0;
				PixelIndex < OutColors.Num();
				++PixelIndex)
			{
				const int32 ByteIndex = PixelIndex * 4;
				OutColors[PixelIndex] = OutPixelFormat == PF_B8G8R8A8
					? FColor(
						Bytes[ByteIndex + 2],
						Bytes[ByteIndex + 1],
						Bytes[ByteIndex],
				...
				for (int32 BlockX = 0; BlockX < BlocksX; ++BlockX)
				{
					...
					for (int32 LocalY = 0; LocalY < 4; ++LocalY)
					{
						for (int32 LocalX = 0; LocalX < 4; ++LocalX)
						{
							...
							const int32 LocalIndex = LocalY * 4 + LocalX;
							OutColors[Y * OutWidth + X] =
								OutPixelFormat == PF_DXT1
									|| OutPixelFormat == PF_DXT5
									? Colors[LocalIndex]
									: OutPixelFormat == PF_BC5
										? FColor(
											FirstChannel[LocalIndex],
											SecondChannel[LocalIndex],
											0,
											255)
										: FColor(
											FirstChannel[LocalIndex],
	...
	bool DecodeReferenceImageColors(
		UTexture2D* Texture,
		TArray<FColor>& OutColors,
		int32& OutWidth,
		int32& OutHeight,
		EPixelFormat& OutPixelFormat)
	{
		if (DecodePlatformTextureColors(
			Texture,
			OutColors,
			OutWidth,
			OutHeight,
	...
	void BuildLowResolutionReferenceMask(
		const TArray<uint8>& SourceIntensity,
		const int32 SourceWidth,
		const int32 SourceHeight,
		TArray<uint8>& OutMask)
	{
		OutMask.SetNumZeroed(
			ForgeryScoreGridResolution * ForgeryScoreGridResolution);
		for (int32 GridY = 0; GridY < ForgeryScoreGridResolution; ++GridY)
		{
			const int32 SourceMinY =
				GridY * SourceHeight / ForgeryScoreGridResolution;
			const int32 SourceMaxY = FMath::Max(
				SourceMinY + 1,
				(GridY + 1) * SourceHeight / ForgeryScoreGridResolution);
			for (int32 GridX = 0; GridX < ForgeryScoreGridResolution; ++GridX)
			{
				const int32 SourceMinX =
					GridX * SourceWidth / ForgeryScoreGridResolution;
				const int32 SourceMaxX = FMath::Max(
					SourceMinX + 1,
					(GridX + 1) * SourceWidth / ForgeryScoreGridResolution);
				uint8 MaximumIntensity = 0;
				for (int32 SourceY = SourceMinY;
					SourceY < FMath::Min(SourceMaxY, SourceHeight);
					++SourceY)
				{
					for (int32 SourceX = SourceMinX;
						SourceX < FMath::Min(SourceMaxX, SourceWidth);
						++SourceX)
					{
						MaximumIntensity = FMath::Max(
							MaximumIntensity,
							SourceIntensity[SourceY * SourceWidth + SourceX]);
					}
				}
				OutMask[GridY * ForgeryScoreGridResolution + GridX] =
					MaximumIntensity >= ReferenceMaskThreshold ? 1 : 0;
			}
		}
	}

	void BuildLowResolutionBackgroundFilteredMask(
		const TArray<FColor>& SourceColors,
		const int32 SourceWidth,
		const int32 SourceHeight,
		const EHeistForgeryBackgroundFilter FilterMode,
		const float BackgroundColorTolerance,
		TArray<uint8>& OutMask)
	{
		OutMask.SetNumZeroed(
			ForgeryScoreGridResolution * ForgeryScoreGridResolution);
		const float ClampedTolerance = FMath::Clamp(
			BackgroundColorTolerance,
			0.0f,
			0.49f);
		const float BlackThreshold = ClampedTolerance * 255.0f;
		const float WhiteThreshold =
			(1.0f - ClampedTolerance) * 255.0f;
		...
		for (int32 GridY = 0; GridY < ForgeryScoreGridResolution; ++GridY)
		{
			...
			for (int32 GridX = 0; GridX < ForgeryScoreGridResolution; ++GridX)
			{
					...
					++SourceY)
				{
					...
					{
						const FColor& Color =
							SourceColors[SourceY * SourceWidth + SourceX];
						RedTotal += Color.R;
						GreenTotal += Color.G;
						BlueTotal += Color.B;
						++SampleCount;
				...
				const float AverageRed =
					static_cast<float>(RedTotal) / SampleCount;
				const float AverageGreen =
					static_cast<float>(GreenTotal) / SampleCount;
				const float AverageBlue =
					static_cast<float>(BlueTotal) / SampleCount;
				const bool bForeground =
					FilterMode == EHeistForgeryBackgroundFilter::Black
						? FMath::Max3(
							AverageRed,
							AverageGreen,
							AverageBlue) > BlackThreshold
						: FilterMode
							== EHeistForgeryBackgroundFilter::White
							? FMath::Min3(
								AverageRed,
								AverageGreen,
								AverageBlue) < WhiteThreshold
							: false;
				OutMask[GridY * ForgeryScoreGridResolution + GridX] =
					bForeground ? 1 : 0;
			}
		...
		const TArray<FLinearColor>& Palette)
	{
		...
		for (int32 PaletteIndex = 0;
			PaletteIndex < Palette.Num();
			++PaletteIndex)
		{
			const FVector3f Delta(
				LinearColor.R - Palette[PaletteIndex].R,
				LinearColor.G - Palette[PaletteIndex].G,
				LinearColor.B - Palette[PaletteIndex].B);
	...
	void BuildLowResolutionReferencePaletteMap(
		const TArray<FColor>& SourceColors,
		const int32 SourceWidth,
		const int32 SourceHeight,
		const TArray<uint8>& ReferenceMask,
		const TArray<FLinearColor>& Palette,
		TArray<uint8>& OutPaletteMap)
	{
		OutPaletteMap.Init(
			EmptyPaletteIndex,
			ForgeryScoreGridResolution * ForgeryScoreGridResolution);
		for (int32 GridY = 0; GridY < ForgeryScoreGridResolution; ++GridY)
		{
			for (int32 GridX = 0; GridX < ForgeryScoreGridResolution; ++GridX)
			{
				const int32 GridIndex =
					GridY * ForgeryScoreGridResolution + GridX;
				if (!ReferenceMask.IsValidIndex(GridIndex)
					|| ReferenceMask[GridIndex] == 0)
				{
					continue;
				}

				const int32 SourceX = FMath::Clamp(
					(GridX * SourceWidth + ForgeryScoreGridResolution / 2)
						/ ForgeryScoreGridResolution,
					0,
					SourceWidth - 1);
				const int32 SourceY = FMath::Clamp(
					(GridY * SourceHeight + ForgeryScoreGridResolution / 2)
						/ ForgeryScoreGridResolution,
					0,
					SourceHeight - 1);
				OutPaletteMap[GridIndex] = FindNearestPaletteIndex(
					SourceColors[SourceY * SourceWidth + SourceX],
					Palette);
			}
		}
	...
	void StampForgeryPaletteBrush(
		TArray<uint8>& PlayerPaletteMap,
		const FVector2D& NormalizedPoint,
		const float BrushSize,
		const uint8 PaletteIndex)
	{
		const float CenterX =
			FMath::Clamp(NormalizedPoint.X, 0.0, 1.0)
			* (ForgeryScoreGridResolution - 1);
		const float CenterY =
			FMath::Clamp(NormalizedPoint.Y, 0.0, 1.0)
			* (ForgeryScoreGridResolution - 1);
		const float RadiusPixels = FMath::Max(
			1.0f,
			BrushSize * ForgeryScoreGridResolution * 0.5f);
		const int32 MinimumX = FMath::Max(
			0,
			FMath::FloorToInt(CenterX - RadiusPixels));
		const int32 MaximumX = FMath::Min(
			ForgeryScoreGridResolution - 1,
			FMath::CeilToInt(CenterX + RadiusPixels));
		const int32 MinimumY = FMath::Max(
			0,
			FMath::FloorToInt(CenterY - RadiusPixels));
		const int32 MaximumY = FMath::Min(
			ForgeryScoreGridResolution - 1,
			FMath::CeilToInt(CenterY + RadiusPixels));
		const float RadiusSquared = FMath::Square(RadiusPixels);
		for (int32 Y = MinimumY; Y <= MaximumY; ++Y)
		{
			for (int32 X = MinimumX; X <= MaximumX; ++X)
			{
				if (FMath::Square(X - CenterX)
					+ FMath::Square(Y - CenterY)
					<= RadiusSquared)
				{
					PlayerPaletteMap[
						Y * ForgeryScoreGridResolution + X] =
						PaletteIndex;
				}
			}
		}
	...
	void RasterizeForgeryPaletteStrokes(
	...
	{
		OutPlayerPaletteMap.Init(
			EmptyPaletteIndex,
			ForgeryScoreGridResolution * ForgeryScoreGridResolution);
		int32 PointOffset = 0;
		for (int32 StrokeIndex = 0;
			StrokeIndex < StrokePointCounts.Num();
			++StrokeIndex)
		{
			const int32 StrokePointCount = StrokePointCounts[StrokeIndex];
			if (StrokePointCount <= 0
				|| PointOffset + StrokePointCount > NormalizedPoints.Num()
				|| !StrokePaletteIndices.IsValidIndex(StrokeIndex))
			{
				break;
			}

			const uint8 PaletteIndex = StrokePaletteIndices[StrokeIndex];
			StampForgeryPaletteBrush(
				OutPlayerPaletteMap,
				NormalizedPoints[PointOffset],
				BrushSize,
				PaletteIndex);
			for (int32 StrokePointIndex = 1;
				StrokePointIndex < StrokePointCount;
				++StrokePointIndex)
			{
				const FVector2D& Start =
					NormalizedPoints[PointOffset + StrokePointIndex - 1];
				const FVector2D& End =
					NormalizedPoints[PointOffset + StrokePointIndex];
				const int32 SampleCount = FMath::Max(
					1,
					FMath::CeilToInt(
						FVector2D::Distance(Start, End)
						* ForgeryScoreGridResolution
						* 2.0f));
...
#if WITH_OPENCV
	bool BuildOpenCVBinaryMasks(
		const TArray<uint8>& ReferenceMask,
		const TArray<uint8>& SubmittedPaletteMap,
		cv::Mat& OutReferenceMask,
		cv::Mat& OutSubmittedMask)
	{
		const int32 ExpectedPixelCount =
			ForgeryScoreGridResolution * ForgeryScoreGridResolution;
		if (ReferenceMask.Num() != ExpectedPixelCount
			|| SubmittedPaletteMap.Num() != ExpectedPixelCount)
		{
			return false;
		}

		OutReferenceMask = cv::Mat::zeros(
			ForgeryScoreGridResolution,
			ForgeryScoreGridResolution,
			CV_8UC1);
		OutSubmittedMask = cv::Mat::zeros(
			ForgeryScoreGridResolution,
			ForgeryScoreGridResolution,
			CV_8UC1);
		for (int32 PixelIndex = 0;
			PixelIndex < ExpectedPixelCount;
			++PixelIndex)
		{
			OutReferenceMask.data[PixelIndex] =
				ReferenceMask[PixelIndex] != 0 ? 255 : 0;
			OutSubmittedMask.data[PixelIndex] =
				SubmittedPaletteMap[PixelIndex] != EmptyPaletteIndex
					? 255
					: 0;
		}
		return true;
	...
	bool BuildOpenCVPaletteImages(
		const TArray<uint8>& ReferencePaletteMap,
		const TArray<uint8>& SubmittedPaletteMap,
		const TArray<FLinearColor>& Palette,
		cv::Mat& OutReferenceBgr,
		cv::Mat& OutSubmittedBgr,
		cv::Mat& OutReferenceHistogram,
		cv::Mat& OutSubmittedHistogram)
	{
		const int32 ExpectedPixelCount =
			ForgeryScoreGridResolution * ForgeryScoreGridResolution;
		if (ReferencePaletteMap.Num() != ExpectedPixelCount
			|| SubmittedPaletteMap.Num() != ExpectedPixelCount
			|| !FMath::IsWithinInclusive(Palette.Num(), 2, 8))
		{
			return false;
		}

		const cv::Scalar NeutralBackground(127, 127, 127);
		OutReferenceBgr = cv::Mat(
			ForgeryScoreGridResolution,
			ForgeryScoreGridResolution,
			CV_8UC3,
			NeutralBackground);
		OutSubmittedBgr = cv::Mat(
			ForgeryScoreGridResolution,
			ForgeryScoreGridResolution,
			CV_8UC3,
			NeutralBackground);
		OutReferenceHistogram = cv::Mat::zeros(
			1,
			Palette.Num(),
			CV_32F);
		OutSubmittedHistogram = cv::Mat::zeros(
			1,
			Palette.Num(),
			CV_32F);
		...
		for (int32 PixelIndex = 0;
			...
			++PixelIndex)
		{
			const uint8 ReferencePaletteIndex =
				ReferencePaletteMap[PixelIndex];
			const uint8 SubmittedPaletteIndex =
				SubmittedPaletteMap[PixelIndex];
			if (ReferencePaletteIndex != EmptyPaletteIndex)
			{
				...
				const FColor Color =
					Palette[ReferencePaletteIndex].ToFColorSRGB();
				OutReferenceBgr.at<cv::Vec3b>(
					PixelIndex / ForgeryScoreGridResolution,
					PixelIndex % ForgeryScoreGridResolution) =
					cv::Vec3b(Color.B, Color.G, Color.R);
				OutReferenceHistogram.at<float>(
					0,
					ReferencePaletteIndex) += 1.0f;
			}
			...
			if (SubmittedPaletteIndex != EmptyPaletteIndex)
			{
				...
				const FColor Color =
					Palette[SubmittedPaletteIndex].ToFColorSRGB();
				OutSubmittedBgr.at<cv::Vec3b>(
					PixelIndex / ForgeryScoreGridResolution,
					PixelIndex % ForgeryScoreGridResolution) =
					cv::Vec3b(Color.B, Color.G, Color.R);
				OutSubmittedHistogram.at<float>(
					0,
					SubmittedPaletteIndex) += 1.0f;
			}
		}

		cv::normalize(
			OutReferenceHistogram,
			OutReferenceHistogram,
			1.0,
			0.0,
			cv::NORM_L1);
		cv::normalize(
			OutSubmittedHistogram,
			OutSubmittedHistogram,
			1.0,
			0.0,
			cv::NORM_L1);
	...
	float CalculateOpenCVDistanceSimilarity(
		...
		const cv::Mat& TargetMask)
	{
		...
		for (int32 Y = 0; Y < SourceMask.rows; ++Y)
		{
			...
			for (int32 X = 0; X < SourceMask.cols; ++X)
			{
				...
				const float NormalizedDistance =
					DistanceRow[X] / OpenCVDistanceTolerancePixels;
				SimilarityTotal += 1.0
		...
		return SampleCount > 0
			? static_cast<float>(SimilarityTotal / SampleCount)
			: 0.0f;
	}

	bool CalculateOpenCVForgeryMetrics(
		const TArray<uint8>& ReferenceMask,
		const TArray<uint8>& ReferencePaletteMap,
		const TArray<uint8>& SubmittedPaletteMap,
		const TArray<FLinearColor>& Palette,
		FOpenCVForgeryMetrics& OutMetrics)
	{
		...
		OutMetrics.ReferencePixelCount =
			cv::countNonZero(ReferenceBinaryMask);
		OutMetrics.SubmittedPixelCount =
			cv::countNonZero(SubmittedBinaryMask);
		if (OutMetrics.ReferencePixelCount <= 0
			|| OutMetrics.SubmittedPixelCount <= 0)
		{
			return false;
		}
		...
		OutMetrics.ReferenceCoverage =
			CalculateOpenCVDistanceSimilarity(
				ReferenceShapeMask,
				SubmittedShapeMask);
		OutMetrics.SubmittedPrecision =
			CalculateOpenCVDistanceSimilarity(
				SubmittedShapeMask,
				ReferenceShapeMask);
		const float ShapeSimilarityTotal =
			OutMetrics.ReferenceCoverage
			+ OutMetrics.SubmittedPrecision;
		OutMetrics.BidirectionalShapeSimilarity =
			ShapeSimilarityTotal > UE_SMALL_NUMBER
				? 2.0f
					* OutMetrics.ReferenceCoverage
					* OutMetrics.SubmittedPrecision
		...
		ComparisonBounds.x = FMath::Max(
			0,
			ComparisonBounds.x - ComparisonPaddingPixels);
		ComparisonBounds.y = FMath::Max(
			0,
			ComparisonBounds.y - ComparisonPaddingPixels);
		ComparisonBounds.width = FMath::Min(
			ForgeryScoreGridResolution - ComparisonBounds.x,
			ComparisonBounds.width + ComparisonPaddingPixels * 2);
		ComparisonBounds.height = FMath::Min(
			ForgeryScoreGridResolution - ComparisonBounds.y,
			ComparisonBounds.height + ComparisonPaddingPixels * 2);

		cv::Mat ReferenceLab;
		cv::Mat SubmittedLab;
		cv::cvtColor(
			ReferenceBgr(ComparisonBounds),
			ReferenceLab,
			cv::COLOR_BGR2Lab);
		cv::cvtColor(
			SubmittedBgr(ComparisonBounds),
			SubmittedLab,
			cv::COLOR_BGR2Lab);
		...
		cv::quality::QualitySSIM::compute(
			ReferenceLab,
			SubmittedLab,
			StructuralSimilarityMap);
		const cv::Scalar StructuralSimilarity = cv::mean(
			StructuralSimilarityMap,
			UnionMask(ComparisonBounds));
		OutMetrics.StructuralColorSimilarity = FMath::Clamp(
			static_cast<float>(
			...
			0.0f,
			1.0f);
		OutMetrics.HistogramColorSimilarity = FMath::Clamp(
			1.0f - static_cast<float>(cv::compareHist(
				ReferenceHistogram,
				SubmittedHistogram,
	...
	float& OutObservationDuration)
{
	...
	if (!FMath::IsWithinInclusive(
			TemplateDefinition.AllowedPalette.Num(),
			2,
			8)
		|| TemplateDefinition.ShapeAccuracyWeight
			+ TemplateDefinition.ColorAccuracyWeight <= 0.0f
		|| TemplateDefinition.MaximumPaintToReferenceRatio < 1.0f
		|| !FMath::IsWithinInclusive(
			TemplateDefinition.BackgroundColorTolerance,
			0.0f,
			0.49f)
		|| !FMath::IsWithinInclusive(
			TemplateDefinition.OverpaintScoreCap,
			0.0f,
			100.0f))
	{
		UE_LOG(
			LogHeist,
			Error,
			TEXT("Forgery template preparation rejected: Character=%s Case=%s Artifact=%s Template=%s PaletteColors=%d Reason=InvalidPaletteScoreContract"),
			*GetNameSafe(HeistCharacter),
			*GetNameSafe(TargetDisplayCase),
			*ArtifactId.ToString(),
	...
	if (TemplateDefinition.BackgroundFilterMode
		== EHeistForgeryBackgroundFilter::None)
	{
		LoadedReferenceMask =
			TemplateDefinition.ReferenceMask.LoadSynchronous();
	}
	if (!IsValid(LoadedReferenceImage)
		|| (TemplateDefinition.BackgroundFilterMode
				== EHeistForgeryBackgroundFilter::None
			&& !IsValid(LoadedReferenceMask)))
	{
		UE_LOG(
			LogHeist,
			Error,
	...
	UE_LOG(
		LogHeist,
		Log,
		TEXT("Forgery template prepared: Character=%s Case=%s Artifact=%s Template=%s ReferenceImage=%s ReferenceMask=%s BackgroundFilter=%s BackgroundTolerance=%.3f PaletteColors=%d Observation=%.2f Forgery=%.2f StrokeLimit=%d Brush=%.4f CoverageWeight=%.3f MajorShapeWeight=%.3f ExtraStrokePenaltyWeight=%.3f TimeoutPenalty=%.3f ShapeWeight=%.3f ColorWeight=%.3f MaxPaintRatio=%.2f OverpaintCap=%.2f Result=PASS"),
		*GetNameSafe(HeistCharacter),
		*GetNameSafe(TargetDisplayCase),
		...
		*StaticEnum<EHeistForgeryBackgroundFilter>()
			->GetNameStringByValue(
				static_cast<int64>(TemplateBackgroundFilterMode)),
		TemplateBackgroundColorTolerance,
		TemplateAllowedPalette.Num(),
...
bool UHeistForgeryComponent::TryBeginSubmit()
{
	...
	if (!ValidateActiveSession(RejectReason)
		|| bSubmitPending
		|| !bHasValidatedStrokePayload)
	{
		UE_LOG(
			LogHeistNetwork,
			Warning,
			...
			*GetNameSafe(ActiveDisplayCase.Get()),
			...
			bSubmitPending
				? TEXT("DuplicateSubmit")
				: !bHasValidatedStrokePayload
					? TEXT("MissingValidatedStrokePayload")
...
bool UHeistForgeryComponent::TrySubmitStrokePayload(
	const TArray<FVector2D>& NormalizedPoints,
	const TArray<int32>& StrokePointCounts,
	const TArray<uint8>& StrokePaletteIndices,
	...
	const int32 ClientSessionRevision)
{
		...
		PayloadBytes))
	{
		...
		UE_LOG(
			LogHeistNetwork,
			Warning,
			TEXT("Forgery stroke payload rejected: Character=%s Case=%s Strokes=%d Points=%d PayloadBytes=%d ClientBrush=%.4f ServerBrush=%.4f ClientSessionRevision=%d ServerSessionRevision=%d SubmitPending=%s Reason=%s"),
			*GetNameSafe(GetOwner()),
			*GetNameSafe(ActiveDisplayCase.Get()),
	...
	if (!TryBeginSubmit())
	{
		...
		RecordStrokeValidationResult(
			false,
			FName(TEXT("SubmitStateTransitionRejected")));
		return false;
	}
	...
	if (!TryCalculateAndCommitForgeryScore())
	{
		...
		RecordStrokeValidationResult(
			false,
			FName(TEXT("ScoreCalculationFailed")));
		++SessionRevision;
		GetOwner()->ForceNetUpdate();
		BroadcastSessionSnapshot(
			TEXT("ServerScoreRejected"),
			FName(TEXT("ScoreCalculationFailed")));
		return false;
	}
	...
	UE_LOG(
		LogHeistNetwork,
		Log,
		TEXT("Forgery stroke payload accepted: Character=%s Case=%s Strokes=%d Points=%d PayloadBytes=%d Brush=%.4f ValidatedForSessionRevision=%d SubmitPending=%s RenderTargetReplicated=false Authority=true Result=PASS"),
		*GetNameSafe(GetOwner()),
		*GetNameSafe(SubmittedDisplayCase),
		ValidatedStrokeCount,
		ValidatedPointCount,
...
bool UHeistForgeryComponent::ForceExpireSubmissionWindowForDebug()
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		SessionEndServerTime =
			ServerWorldTime - SubmissionTimeoutToleranceSeconds - 0.1f;
		GetOwner()->ForceNetUpdate();
		return true;
	}
#endif
...
UHeistForgeryComponent::GetTemplateAllowedPalette() const
{
...
const TArray<int32>&
UHeistForgeryComponent::GetValidatedStrokePointCounts() const
{
	return ValidatedStrokePointCounts;
}

const TArray<uint8>&
UHeistForgeryComponent::GetValidatedStrokePaletteIndices() const
{
...
const FHeistForgeryResult&
UHeistForgeryComponent::GetAuthoritativeForgeryResult() const
{
...
bool UHeistForgeryComponent::RecalculateValidatedForgeryScoreForDebug(
...
bool UHeistForgeryComponent::ValidateStrokePayload(
	const TArray<FVector2D>& NormalizedPoints,
	const TArray<int32>& StrokePointCounts,
	const TArray<uint8>& StrokePaletteIndices,
	const float ClientBrushSize,
	...
	int32& OutPayloadBytes) const
{
	...
	const int64 EstimatedPayloadBytes =
		static_cast<int64>(NormalizedPoints.Num()) * sizeof(FVector2D)
		+ static_cast<int64>(StrokePointCounts.Num()) * sizeof(int32)
		+ static_cast<int64>(StrokePaletteIndices.Num()) * sizeof(uint8)
		+ sizeof(ClientBrushSize)
		+ sizeof(ClientSessionRevision);
	OutPayloadBytes = EstimatedPayloadBytes > MAX_int32
		? MAX_int32
		: static_cast<int32>(EstimatedPayloadBytes);
	...
	const float ServerWorldTime = IsValid(HeistGameState)
		? HeistGameState->GetServerWorldTimeSeconds()
		: (GetWorld() ? GetWorld()->GetTimeSeconds() : 0.0f);
	if (SessionEndServerTime <= 0.0f
		|| ServerWorldTime
			> SessionEndServerTime + SubmissionTimeoutToleranceSeconds)
	{
	...
	if (EstimatedPayloadBytes <= 0
		|| EstimatedPayloadBytes > MaximumStrokePayloadBytes)
	{
	...
	for (const FVector2D& Point : NormalizedPoints)
	{
		...
		if (!FMath::IsWithinInclusive(Point.X, 0.0, 1.0)
	...
	if (!FMath::IsFinite(ClientBrushSize)
		|| !FMath::IsWithinInclusive(ClientBrushSize, 0.001f, 0.25f))
	{
...
void UHeistForgeryComponent::RecordStrokeValidationResult(
	...
	const FName Reason)
{
	...
	UE_LOG(
		...
		*GetNameSafe(GetOwner()),
		...
		bHasValidatedStrokePayload ? TEXT("true") : TEXT("false"),
		ValidatedStrokeCount,
		ValidatedPointCount,
		ValidatedPayloadBytes,
		ValidatedBrushSize,
...
void UHeistForgeryComponent::ResetStrokeTransportState(
	const bool bResetLastValidation)
{
...
bool UHeistForgeryComponent::TryCalculateAndCommitForgeryScore()
{
	...
	if (!CalculateForgeryScore(
		ValidatedStrokePoints,
		ValidatedStrokePointCounts,
		ValidatedStrokePaletteIndices,
		ValidatedBrushSize,
		CalculatedResult,
		CalculatedReferenceMaskPixels,
		CalculatedSubmittedMaskPixels))
	{
		UE_LOG(
			LogHeistNetwork,
			Error,
			TEXT("Forgery score calculation rejected: Character=%s Case=%s Artifact=%s Template=%s ReferenceMask=%s Reason=MaskDecodeOrScoreContractFailed"),
			*GetNameSafe(GetOwner()),
			*GetNameSafe(ActiveDisplayCase.Get()),
			*ActiveArtifactId.ToString(),
			*ActiveTemplateId.ToString(),
	...
	const double ScoreCalculationMilliseconds =
		(FPlatformTime::Seconds() - ScoreCalculationStartSeconds) * 1000.0;
	...
	if (!IsValid(HeistPlayerState) || !IsValid(TargetDisplayCase))
	{
		UE_LOG(
			LogHeistNetwork,
			Error,
			TEXT("Forgery score commit rejected: Character=%s Case=%s Reason=MissingOwnerOrDisplayCase"),
			*GetNameSafe(GetOwner()),
			*GetNameSafe(TargetDisplayCase));
		return false;
	}
	...
	if (!bReplicaCommitted)
	{
		UE_LOG(
			LogHeistNetwork,
			Error,
			TEXT("Forgery score commit rejected: Character=%s Case=%s Reason=ReplicaPlacementRejected"),
			*GetNameSafe(GetOwner()),
			*GetNameSafe(TargetDisplayCase));
		return false;
	}

	AuthoritativeForgeryResult =
		TargetDisplayCase->GetCommittedForgeryResult();
	...
	UE_LOG(
		...
		*AuthoritativeForgeryResult.TemplateId.ToString(),
		AuthoritativeForgeryResult.SimilarityScore,
		AuthoritativeForgeryResult.CoverageScore,
		AuthoritativeForgeryResult.MajorShapeScore,
		AuthoritativeForgeryResult.ColorAccuracyScore,
		AuthoritativeForgeryResult.MissingShapePenalty,
		AuthoritativeForgeryResult.ExtraStrokePenalty,
		AuthoritativeForgeryResult.TimeoutPenalty,
		AuthoritativeForgeryResult.CompletionTime,
		AuthoritativeForgeryResult.PaintToReferenceRatio,
		FMath::Pow(
			FMath::Clamp(
				AuthoritativeForgeryResult.PaintToReferenceRatio,
				0.0f,
		...
		AuthoritativeForgeryResult.bAntiFillTriggered
			? TEXT("true")
			: TEXT("false"),
		ForgeryScoreResolution,
		ForgeryScoreResolution,
		ReferenceMaskPixelCount,
		SubmittedMaskPixelCount,
		static_cast<float>(ReferenceMaskPixelCount) / TotalScorePixels,
		static_cast<float>(SubmittedMaskPixelCount) / TotalScorePixels,
		bLastScoringReferenceCacheHit ? TEXT("Hit") : TEXT("Miss"),
		LastScoringReferenceMilliseconds,
		LastOpenCVScoringMilliseconds,
		ScoreCalculationMilliseconds,
		ForgeryScoreRevision);
...
bool UHeistForgeryComponent::BuildScoringReferenceCache() const
{
	...
	if (TemplateBackgroundFilterMode
		== EHeistForgeryBackgroundFilter::None)
	{
		UTexture2D* ReferenceMaskTexture =
			ReferenceMaskAsset.LoadSynchronous();
		if (!DecodeReferenceMaskIntensity(
			ReferenceMaskTexture,
			SourceIntensity,
			SourceWidth,
			SourceHeight,
			SourcePixelFormat))
		{
			UE_LOG(
				LogHeistNetwork,
				Error,
				TEXT("Forgery score mask decode failed: Character=%s Texture=%s PixelFormat=%d Source=%dx%d"),
				*GetNameSafe(GetOwner()),
				*GetNameSafe(ReferenceMaskTexture),
				static_cast<int32>(SourcePixelFormat),
				SourceWidth,
				SourceHeight);
			return false;
		}
	...
	UTexture2D* ReferenceImageTexture =
		ReferenceImageAsset.LoadSynchronous();
	TArray<FColor> SourceColors;
	int32 ImageWidth = 0;
	int32 ImageHeight = 0;
	EPixelFormat ImagePixelFormat = PF_Unknown;
	if (!DecodeReferenceImageColors(
		...
		ImagePixelFormat))
	{
		UE_LOG(
			LogHeistNetwork,
			Error,
			TEXT("Forgery score image decode failed: Character=%s Texture=%s PixelFormat=%d Source=%dx%d"),
			*GetNameSafe(GetOwner()),
			*GetNameSafe(ReferenceImageTexture),
			static_cast<int32>(ImagePixelFormat),
			ImageWidth,
			ImageHeight);
		return false;
	}
	...
	if (TemplateBackgroundFilterMode
		== EHeistForgeryBackgroundFilter::None)
	{
		BuildLowResolutionReferenceMask(
			SourceIntensity,
			SourceWidth,
			SourceHeight,
			CachedReferenceMask);
	...
	{
		BuildLowResolutionBackgroundFilteredMask(
			SourceColors,
			ImageWidth,
			ImageHeight,
			TemplateBackgroundFilterMode,
	...
	int32& OutSubmittedMaskPixels) const
{
	...
	const int32 ExpectedScorePixelCount =
		ForgeryScoreGridResolution * ForgeryScoreGridResolution;
	bLastScoringReferenceCacheHit =
		CachedScoringTemplateId == ActiveTemplateId
		&& CachedReferenceMask.Num() == ExpectedScorePixelCount
		&& CachedReferencePaletteMap.Num() == ExpectedScorePixelCount;
	...
	{
		LastScoringReferenceMilliseconds =
			(FPlatformTime::Seconds() - ReferenceScoringStartSeconds)
			* 1000.0;
		return false;
	}
	LastScoringReferenceMilliseconds =
	...
	if (!CalculateOpenCVForgeryMetrics(
		ReferenceMask,
		ReferencePaletteMap,
		SubmittedPaletteMap,
		TemplateAllowedPalette,
	...
	{
		LastOpenCVScoringMilliseconds =
			(FPlatformTime::Seconds() - OpenCVScoringStartSeconds)
			* 1000.0;
		return false;
	}
	LastOpenCVScoringMilliseconds =
	...
	UE_LOG(
		LogHeistNetwork,
		Error,
		TEXT("Forgery score calculation rejected: Character=%s Template=%s Reason=OpenCVUnavailable"),
		*GetNameSafe(GetOwner()),
		*ActiveTemplateId.ToString());
	return false;
#endif

	const int32 TotalPixelCount =
		ForgeryScoreGridResolution * ForgeryScoreGridResolution;
	if (OutReferenceMaskPixels <= 0
		|| OutReferenceMaskPixels >= TotalPixelCount)
	{
		...
		UE_LOG(
			LogHeistNetwork,
			Error,
			TEXT("Forgery score mask rejected: Character=%s Texture=%s ReferencePixels=%d TotalPixels=%d Reason=DegenerateReferenceMask"),
			*GetNameSafe(GetOwner()),
			*MaskSource,
			OutReferenceMaskPixels,
			TotalPixelCount);
	...
	const float CoveragePoints =
		CoverageRatio * TemplateCoverageWeight * 100.0f;
	const float MajorShapePoints =
		MajorShapeRatio * TemplateMajorShapeWeight * 100.0f;
	const float MissingShapePenaltyPoints =
		MissingShapeRatio * TemplateExtraStrokePenaltyWeight * 100.0f;
	...
	float FinalScore = FMath::Clamp(
		...
		0.0f,
		100.0f);
	const float PaintToReferenceRatio =
		static_cast<float>(OutSubmittedMaskPixels)
		/ OutReferenceMaskPixels;
	const float PaintCompletenessFactor = FMath::Pow(
	...
	const float ServerWorldTime = IsValid(HeistGameState)
		? HeistGameState->GetServerWorldTimeSeconds()
		: (GetWorld() ? GetWorld()->GetTimeSeconds() : 0.0f);
	const float SessionStartServerTime =
		SessionEndServerTime - ActiveSessionDurationSeconds;
	const float CompletionTime = FMath::Clamp(
		ServerWorldTime - SessionStartServerTime,
		0.0f,
		ActiveSessionDurationSeconds);
	const auto RoundScore = [](const float Value)
	{
	...
	OutResult.PaintToReferenceRatio =
		RoundScore(PaintToReferenceRatio);
	OutResult.bAntiFillTriggered = bAntiFillTriggered;
	OutResult.MissingShapePenalty =
		RoundScore(MissingShapePenaltyPoints);
	OutResult.ExtraStrokePenalty =
		RoundScore(ExtraStrokePenaltyPoints);
...
bool UHeistForgeryComponent::RunOpenCVScoringSelfTestForDebug(
	FString& OutSummary) const
{
	...
	for (int32 Y = ShapeMinimum; Y <= ShapeMaximum; ++Y)
	{
		for (int32 X = ShapeMinimum; X <= ShapeMaximum; ++X)
		{
			const int32 PixelIndex =
				Y * ForgeryScoreGridResolution + X;
			const uint8 PaletteIndex =
				X < ForgeryScoreGridResolution / 2 ? 0 : 1;
			...
			{
				ShiftedPaletteMap[
					Y * ForgeryScoreGridResolution + ShiftedX] =
					PaletteIndex;
			}
		}
	}

	const TArray<FLinearColor> TestPalette = {
		FLinearColor(0.95f, 0.08f, 0.05f, 1.0f),
		FLinearColor(0.04f, 0.15f, 0.95f, 1.0f)
	};
	...
	const bool bCalculated =
		...
		&& CalculateOpenCVForgeryMetrics(
			...
			TestPalette,
			FilledMetrics);
	const float FilledAreaRatio = bCalculated
...
void UHeistForgeryComponent::CompleteSuccessfulForgerySession()
{
	...
	UE_LOG(
		LogHeistNetwork,
		Log,
		TEXT("Forgery session completed: Character=%s PreviousCase=%s HasScore=%s Score=%.2f ReplicaPlaced=%s Revision=%d Authority=true Result=%s"),
		*GetNameSafe(GetOwner()),
		*GetNameSafe(PreviousDisplayCase),
		bHasAuthoritativeForgeryResult ? TEXT("true") : TEXT("false"),
		AuthoritativeForgeryResult.SimilarityScore,
		AuthoritativeForgeryResult.bReplicaPlaced ? TEXT("true") : TEXT("false"),
		SessionRevision,
		bHasAuthoritativeForgeryResult
			&& AuthoritativeForgeryResult.bReplicaPlaced
			? TEXT("PASS")
			: TEXT("FAIL"));
	BroadcastSessionSnapshot(
		TEXT("ServerComplete"),
	...
	const FName Reason)
{
	...
	UE_LOG(
		LogHeistNetwork,
		Log,
		TEXT("Forgery session %s: Character=%s Case=%s Active=%s SubmitPending=%s HasScore=%s Score=%.2f TemplatePrepared=%s Artifact=%s Template=%s EndServerTime=%.2f Revision=%d LastCleanup=%s Reason=%s Authority=%s"),
		ChangeSource,
		*GetNameSafe(GetOwner()),
		*GetNameSafe(ActiveDisplayCase.Get()),
...
void UHeistForgeryComponent::OnRep_StrokeValidationRevision()
{
	UE_LOG(
		LogHeistNetwork,
		Log,
		TEXT("Forgery stroke validation replicated: Character=%s Accepted=%s HasValidatedPayload=%s Strokes=%d Points=%d PayloadBytes=%d Brush=%.4f Reason=%s ValidationRevision=%d Authority=false Result=%s"),
		*GetNameSafe(GetOwner()),
		bLastStrokeValidationAccepted ? TEXT("true") : TEXT("false"),
		bHasValidatedStrokePayload ? TEXT("true") : TEXT("false"),
		ValidatedStrokeCount,
		ValidatedPointCount,
		ValidatedPayloadBytes,
		ValidatedBrushSize,
		LastStrokeValidationReason.IsNone()
			? TEXT("None")
			: *LastStrokeValidationReason.ToString(),
		StrokeValidationRevision,
...
void UHeistForgeryComponent::OnRep_ForgeryScoreRevision()
{
	...
	UE_LOG(
		LogHeistNetwork,
		Log,
		TEXT("Forgery score replicated: Character=%s HasScore=%s Artifact=%s Template=%s Score=%.2f Coverage=%.2f MajorShape=%.2f ColorAccuracy=%.2f MissingPenalty=%.2f ExtraPenalty=%.2f TimeoutPenalty=%.2f CompletionTime=%.2f PaintToReference=%.2f AntiFill=%s Resolution=%dx%d ReferencePixels=%d SubmittedPixels=%d ScoreRevision=%d Authority=false Result=%s"),
		*GetNameSafe(GetOwner()),
		bHasAuthoritativeForgeryResult ? TEXT("true") : TEXT("false"),
		*AuthoritativeForgeryResult.ArtifactId.ToString(),
		*AuthoritativeForgeryResult.TemplateId.ToString(),
		...
		SubmittedMaskPixelCount,
		ForgeryScoreRevision,
		bHasAuthoritativeForgeryResult ? TEXT("PASS") : TEXT("CLEARED"));
}
	...
	TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(
		UHeistForgeryComponent,
		bLastStrokeValidationAccepted,
		COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(
		UHeistForgeryComponent,
		LastStrokeValidationReason,
		COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(
		UHeistForgeryComponent,
		StrokeValidationRevision,
