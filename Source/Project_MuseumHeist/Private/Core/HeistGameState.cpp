...
FText AHeistGameState::GetContractOutcomeReasonText() const
{
...
bool AHeistGameState::CommitContractOutcome(const EHeistContractOutcome Outcome, const FName OutcomeReasonId)
{
	...
	ContractSnapshot.OutcomeReasonId = OutcomeReasonId;
	ContractSnapshot.Revision = ContractSnapshot.Revision == MAX_int32 ? 1 : ContractSnapshot.Revision + 1;
	ForceNetUpdate();
...
bool AHeistGameState::CommitTeamResult(FHeistTeamResult NewTeamResult)
{
	if (!HasAuthority() || ContractSnapshot.Outcome == EHeistContractOutcome::None || NewTeamResult.Outcome != ContractSnapshot.Outcome ||
		NewTeamResult.OutcomeReasonId != ContractSnapshot.OutcomeReasonId || NewTeamResult.LootValueQuota != ContractSnapshot.LootValueQuota ||
...
const FHeistTeamResult& AHeistGameState::GetTeamResult() const
{
	return TeamResult;
}

FHeistTeamResultChanged& AHeistGameState::GetTeamResultChangedDelegate()
{
	return TeamResultChangedDelegate;
}
...
void AHeistGameState::RebuildPlayerResults()
{
	...
	for (APlayerState* PlayerState : PlayerArray)
	{
		...
		PlayerResult.bArrested = HeistPlayerState->IsArrested() || HeistPlayerState->IsProtectedByDetention();
		PlayerResult.Contribution = HeistPlayerState->GetContribution();
		PlayerResult.Contribution.bEscaped = PlayerResult.bEscaped;
		PlayerResult.Contribution.bArrested = PlayerResult.bArrested;
	}
	...
	const bool bAllResolved = AreAllCrewMembersResolved();
	const bool bAllRemainingArrested = AreAllRemainingCrewMembersArrested();
	UE_LOG(LogHeist, Log, TEXT("Team player lifecycle: Players=%d Arrested=%d Escaped=%d Active=%d AllResolved=%s AllRemainingArrested=%s FailureEligible=%s Authority=true"), TotalPlayerCount,
		   ...
		   bAllResolved && bAllRemainingArrested ? TEXT("true") : TEXT("false"));
...
bool AHeistGameState::AreAllCrewMembersResolved() const
{
	...
	{
		const AHeistPlayerState* HeistPlayerState = Cast<AHeistPlayerState>(PlayerState);
		if (IsValid(HeistPlayerState) && (HeistPlayerState->IsDetentionRecoveryPending() ||
			(GetMatchPhase() == EHeistMatchPhase::InGame && HeistPlayerState->IsProtectedByDetention())))
		{
			return false;
		}
	}
...
void AHeistGameState::GetPlayerLifecycleCounts(int32& OutTotalPlayers, int32& OutActivePlayers, int32& OutEscapedPlayers, int32& OutArrestedPlayers) const
{
	...
	for (const APlayerState* PlayerState : PlayerArray)
	{
		...
		else if (HeistPlayerState->IsArrested() || HeistPlayerState->IsProtectedByDetention())
		{
			++OutArrestedPlayers;
		}
		else
