...
UHeistInventorySlotWidget::UHeistInventorySlotWidget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UHeistInventorySlotWidget::SetupSlot(
...
void UHeistInventorySlotWidget::NativeOnDragEnter(
	...
	UDragDropOperation* InOperation)
{
	...
	if (IsValid(InventoryOperation) && IsValid(InventoryWidget))
	{
		SetDropPreview(
			true,
			InventoryWidget->CanPreviewItemDrop(
				InventoryOperation->InstanceId,
				GridCoordinate));
	}
}

void UHeistInventorySlotWidget::NativeOnDragLeave(
	const FDragDropEvent& InDragDropEvent,
	UDragDropOperation* InOperation)
{
...
bool UHeistInventorySlotWidget::NativeOnDrop(
	const FGeometry& InGeometry,
	const FDragDropEvent& InDragDropEvent,
	UDragDropOperation* InOperation)
{
	...
	if (bHandled)
	{
		InventoryWidget->RequestMoveItem(
			InventoryOperation->InstanceId,
			GridCoordinate);
	}
...
void UHeistInventorySlotWidget::RefreshPresentation()
{
	...
	{
		CoordinateText->SetText(FText::Format(
			NSLOCTEXT("HeistInventory", "SlotCoordinateFormat", "{0},{1}"),
			FText::AsNumber(GridCoordinate.X),
			FText::AsNumber(GridCoordinate.Y)));
	}
	...
	if (IsValid(OccupancyText))
	{
		OccupancyText->SetText(
			bOccupied
				? NSLOCTEXT("HeistInventory", "SlotOccupied", "OCC")
				: FText::GetEmpty());
	}

	if (IsValid(SlotBackground))
	{
		FLinearColor SlotColor = bOccupied
			? FLinearColor(0.10f, 0.16f, 0.22f, 0.90f)
			: FLinearColor(0.04f, 0.07f, 0.10f, 0.85f);
		if (bDropPreviewVisible)
		{
			SlotColor = bDropPreviewValid
				? FLinearColor(0.06f, 0.55f, 0.20f, 0.95f)
				: FLinearColor(0.70f, 0.06f, 0.08f, 0.95f);
		}
