...
{
    Join-Path $projectRoot "SourceArt\Forgery\$PoolId"
}
else
{
...
{
    Join-Path $projectRoot "DataTableImports\DT_ForgeryTemplateRow.json"
}
else
{
    [System.IO.Path]::GetFullPath($DataTablePath)
}
$artifactDataTablePath = Join-Path $projectRoot "DataTableImports\DT_ArtifactDataRow.json"
